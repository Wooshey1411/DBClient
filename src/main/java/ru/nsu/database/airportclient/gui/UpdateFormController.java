...
public class UpdateFormController {
    ...
    public void init(List<Token> infoTokens){
        nodes = new ArrayList<>();
        ...
        primaryTokens = new ArrayList<>();
        for (Token token : infoTokens){
            if(token.isPK()){
                primaryTokens.add(token);
            }
        }

        for (LinkedToken linkedToken : context.getCurrentTable().getTokensForUpdate()){
            ...
            String data = null;
            for (Token token : infoTokens){
                if (token.name().equals(linkedToken.name())){
                    data = token.data();
                    break;
                }
            }
            ...
            if(linkedToken.request() != null){
                ...
                try{
                    ...
                    for (Token vT : infoTokens){
                        if (vT.name().equals(primaryToken.name())){
                            valueToken = vT;
                            break;
                        }
                    }
                    ...
                    for (List<Token> row : reqTokens){
                        ...
                        if(valueToken != null) {
                            if (reqTokens.get(currRow).get(primaryTokenPos).data().equals(valueToken.data())){
                                comboBox.setValue(res.toString());
                            }
                        }
                        currRow++;
                    }
    ...
    private void onUpdateBtnClick(){
        ...
        try(PreparedStatement statement = DBConnection.INSTANCE.getConnection().prepareStatement(context.getCurrentTable().getSQLForUpdate())) {
            ...
            for (Token token : primaryTokens){
                StatementCreator.fillStatementField(token.data(), token.tokenType(), statement, pos+1);
                pos++;
            }

            statement.execute();

            Alert alert = new Alert(Alert.AlertType.INFORMATION, "Успешно обновлено", ButtonType.OK);
            alert.showAndWait();
