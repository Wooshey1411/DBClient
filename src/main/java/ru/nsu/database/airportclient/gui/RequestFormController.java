...
public class RequestFormController {
    ...
    private List<Token> primaryTokens;
    ...
    public void init(){
        ...
        for (LinkedToken linkedToken : context.getCurrentRequest().getTokensForRequest()){
    ...
    private void onUpdateBtnClick(){
        ...
        StringBuilder sqlRequest = new StringBuilder(context.getCurrentRequest().getRequestBody());

        if(!context.getCurrentRequest().getRequestName().equals("get_airplanes_in_airport")) {

            boolean isFirstVal = true;
            pos = 0;
            for (InfoNode infoNode : nodes) {
                if (infoNode.getInfo() == null || infoNode.getInfo().isEmpty()) {
                    pos++;
                    continue;
                }

                if (isFirstVal) {
                    sqlRequest.append(" where ").append(context.getCurrentRequest().getRequestCond().get(pos));
                    isFirstVal = false;
                } else {
                    sqlRequest.append(" and ").append(context.getCurrentRequest().getRequestCond().get(pos));
                }
        ...
        try(PreparedStatement statement = DBConnection.INSTANCE.getConnection().prepareStatement(sqlRequest.toString())) {
            ...
            for (LinkedToken linkedToken : context.getCurrentRequest().getTokensForRequest()){
                ...
                StatementCreator.fillStatementField(data, linkedToken.type(), statement, queryPos);
                queryPos++;
            ...
            if (context.getCurrentRequest().getRequestName().equals("get_airplanes_in_airport")){
                StatementCreator.fillStatementField(nodes.get(0).getInfo(), TokenType.TIMESTAMP, statement, 2);
            }
            ...
            if(tokens.isEmpty()){
                ...
                mainPane.setContent(borderPane);
            ...
            Stage currStage = (Stage)scrollPane.getScene().getWindow();
            currStage.close();
    ...
    public void setRowsCountLabel(Label rowsCountLabel) {
        this.rowsCountLabel = rowsCountLabel;
    }
}
